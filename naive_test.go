...
func TestNBFSetByte(t *testing.T) {
	bf, err := NewNaiveBloomFilter(1048576, 4)
	assert.Nil(t, err)
	err = bf.setByte(100)
	assert.Nil(t, err)
...
func TestNewNaiveBloomFilter(t *testing.T) {
	bf, err := NewNaiveBloomFilter(100000, 4)
	assert.NotNil(t, err)
	assert.Nil(t, bf)
...
func TestNaiveBloomFilter(t *testing.T) {
	bf, err := NewNaiveBloomFilter(1048576, 4)
	assert.Nil(t, err)
