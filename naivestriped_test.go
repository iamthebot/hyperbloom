...
func TestNSBFSetByte(t *testing.T) {
	bf, err := NewNaiveStripedBloomFilter(1048576, 4, 64)
	assert.Nil(t, err)
	err = bf.setByte(100)
	assert.Nil(t, err)

	bitExists, err := bf.getByte(100)
	assert.Nil(t, err)
	assert.Equal(t, true, bitExists)

	falseBit, err := bf.getByte(1048575)
	assert.Nil(t, err)
	assert.Equal(t, false, falseBit)
}

func TestNewNaiveStripedBloomFilter(t *testing.T) {
	bf, err := NewNaiveStripedBloomFilter(100000, 4, 10)
	assert.NotNil(t, err)
	assert.Nil(t, bf)

	bf, err = NewNaiveStripedBloomFilter(1048576, 4, 10)
	assert.NotNil(t, err)
	assert.Nil(t, bf)
...
func TestNaiveStripedBloomFilter(t *testing.T) {
	bf, err := NewNaiveStripedBloomFilter(1048576, 4, 64)
	assert.Nil(t, err)
